...
FOLLOWUP_UNIQUE_WORDS = [
    ...
    "observed", "provide", "patient", "information",
    "discrepancy", "found", "were",
]
...
def classify_email(row):
    ...
    # ── Rule 2: PPM Strong — fires before CQA ─────────────────────────────────
    if ppm_strong_hits:
    ...
    # ── Rule 3: CQA — receipt + complaint + revert/findings ───────────────────
    # Stronger rule: needs receipt + complaint + investigation signal
    if has_cqa_required and has_cqa_invest:
        return pd.Series({
    ...
    # ── Rule 4: PPM Weak — only without CQA signal ───────────────────────────
